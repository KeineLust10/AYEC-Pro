...
VENDOR_EMAIL = os.environ.get("AYEC_VENDOR_EMAIL", "ayecpro@gmail.com").strip().lower()
VENDOR_PHONE = os.environ.get("AYEC_VENDOR_PHONE", "05348781047").strip()
PUBLIC_SERVER_URL = os.environ.get("AYEC_PUBLIC_URL", "http://85.117.239.60").strip().rstrip("/")
...
LICENSE_PLAN_DEFAULTS = (
    ("monthly", "Ayl\u0131k Esnek", 1, 990.0),
    ("six_months", "6 Ayl\u0131k Avantaj", 6, 5490.0),
    ("one_year", "1 Y\u0131ll\u0131k Profesyonel", 12, 9900.0),
    ("two_years", "2 Y\u0131ll\u0131k \u0130\u015fletme", 24, 17900.0),
    ("three_years", "3 Y\u0131ll\u0131k Kurumsal", 36, 24900.0),
)
...
def ensure_packaged_source() -> None:
    """Make the packaged engine importable without changing package precedence."""
    root_text = str(ROOT)
...
def registry_connect() -> sqlite3.Connection:
    ...
    conn.execute(
        ...
        "VALUES (1,?,?,?,?,?)",
        (LICENSE_PAYMENT_BANK, LICENSE_PAYMENT_HOLDER, LICENSE_PAYMENT_IBAN, VENDOR_EMAIL,
...
def license_payment_profile() -> dict:
    ...
    return {
        ...
        "notification_email": str(profile.get("notification_email") or VENDOR_EMAIL),
    }
...
def send_license_email(tenant_id: str, recipient: str, subject: str, text: str) -> tuple[bool, str]:
    ...
    try:
        ...
        logger.warning("License email delivery failed: %s", error)
...
def ensure_control_owner() -> None:
    """Assign the configured vendor account as platform owner when available."""
    with closing(registry_connect()) as registry:
        vendor_owner_found = False
        for tenant in rows(registry, "SELECT * FROM tenants WHERE COALESCE(active,1)=1 ORDER BY created_at,id"):
            ...
            if owner:
                ...
                vendor_owner_found = True
        if vendor_owner_found:
            registry.commit()
            return
...
def _store_support_backup_for_tenant(
    ...
    filename = f"{datetime.now().strftime('%Y%m%d-%H%M%S')}-{digest[:12]}.db"
    ...
    with closing(registry_connect()) as conn:
        existing = conn.execute(
            "SELECT id FROM support_backups WHERE tenant_id=? AND sha256=?", (tenant_id, digest)
        ).fetchone()
        if existing:
    ...
    return {"ok": True, "backup_id": backup_id, "sha256": digest, "size_bytes": len(payload)}


def support_store_backup(user: dict, payload: bytes, original_name: str = "desktop.db") -> dict:
    tenant_id = str(dict(user).get("_tenant_id") or active_tenant_id() or "")
    return _store_support_backup_for_tenant(
        tenant_id,
...
def control_queue_restore(actor: dict, data: dict) -> dict:
    ...
    with closing(registry_connect()) as conn:
        ...
        cursor = conn.execute(
            ...
            "created_by_tenant_id,created_by_user_id,created_at) VALUES (?, 'restore_backup', ?, 'pending', ?, ?, ?)",
            (
                tenant_id,
                json.dumps({"backup_id": backup_id}, ensure_ascii=True),
                str(item.get("_tenant_id") or ""),
                int(item.get("id") or 0),
                utc_now().isoformat(timespec="seconds"),
            ),
...
def desktop_pending_commands(user: dict) -> dict:
    tenant_id = str(dict(user).get("_tenant_id") or active_tenant_id() or "")
    ...
    with closing(registry_connect()) as conn:
        pending = rows(
            ...
            "WHERE tenant_id=? AND (status='pending' OR (status='claimed' AND claimed_at<?)) "
...
def desktop_complete_command(user: dict, data: dict) -> dict:
    tenant_id = str(dict(user).get("_tenant_id") or active_tenant_id() or "")
    ...
    with closing(registry_connect()) as conn:
        cursor = conn.execute(
            ...
            "WHERE id=? AND tenant_id=? AND status IN ('claimed','pending')",
            (
...
def support_backup_file(user: dict, backup_id: int) -> tuple[bytes, str, str]:
    tenant_id = str(dict(user).get("_tenant_id") or active_tenant_id() or "")
    with closing(registry_connect()) as conn:
        row = conn.execute(
            "SELECT * FROM support_backups WHERE id=? AND tenant_id=? AND status='available'",
...
def admin_list_companies(search: str = "") -> list:
...
def admin_list_licenses() -> list:
    ...
    with closing(registry_connect()) as conn:
        tenant_list = rows(conn, "SELECT id, company_name, license_type, license_start, license_end, license_status, license_updated_at, active FROM tenants ORDER BY created_at, id")
    result = []
...
def create_license_order(user: dict, data: dict) -> dict:
    ...
    with closing(registry_connect()) as conn:
        ...
        cursor = conn.execute(
            ...
            "(request_no,tenant_id,requester_user_id,requester_name,requester_email,requester_phone,hardware_id,"
            "plan_code,plan_label,duration_months,amount_try,currency,payment_reference,status,created_at,updated_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (
                request_no, tenant_id, int(user.get("id") or 0),
    ...
    admin_sent, admin_note = send_license_email(
        tenant_id, VENDOR_EMAIL, admin_subject,
...
def report_license_payment(user: dict, order_id: int, data: dict) -> dict:
    ...
    with closing(registry_connect()) as conn:
        ...
        conn.commit()
    admin_sent, admin_note = send_license_email(
        tenant_id, VENDOR_EMAIL, "AYEC Pro Odeme Bildirimi",
...
def list_license_orders(tenant_id: str = "", limit: int = 250, status: str = "") -> list[dict]:
...
def approve_license_order(order_id: int, data: dict, actor: dict) -> dict:
    ...
    with closing(registry_connect()) as conn:
        ...
        conn.execute(
            "UPDATE tenants SET active=1,license_type=?,license_status='Active',license_start=?,"
            "license_end=?,license_updated_at=? WHERE id=?",
            (
                plan["label"], now, end.isoformat(timespec="seconds"), now,
                str(row["tenant_id"]),
            ),
        )
        conn.execute(
            "UPDATE license_orders SET plan_code=?,plan_label=?,duration_months=?,amount_try=?,status='active',admin_note=?,"
            "approved_by_user_id=?,approved_at=?,license_start=?,license_end=?,updated_at=? WHERE id=?",
            (plan["code"], plan["label"], plan["duration_months"], plan["amount_try"], admin_note,
    ...
    _support_audit(actor, "license_order_approved", tenant_id, detail={"order_id": int(order_id), "plan": plan["code"]})
    return {"ok": True, "order": _license_request_payload(item), "license": tenant_access_summary(tenant_by_id(tenant_id))}
...
def license_user_from_device(data: dict) -> dict:
    ...
    with closing(registry_connect()) as conn:
        tenants = [dict(row) for row in conn.execute(
            "SELECT * FROM tenants WHERE COALESCE(active,1)=1 ORDER BY company_name"
        )]
    matches = []
...
def create_license_order_from_device(data: dict) -> dict:
    return create_license_order(license_user_from_device(data), data)


def license_status_from_device(data: dict) -> dict:
    return license_status_for_user(license_user_from_device(data))
...
def admin_list_backups(tenant_id: str) -> list:
    ...
    with closing(registry_connect()) as conn:
        support_rows = rows(
            conn,
            "SELECT id,original_name,size_bytes,source,created_at,status "
...
def create_tenant(
    ...
    with closing(registry_connect()) as conn:
        ...
        conn.execute(
            "INSERT INTO tenants(id,company_name,db_filename,created_at,active,sector,license_type,license_start,license_end,license_status,contact_name,phone,email,company_address,installation_lat,installation_lng,location_updated_at) VALUES (?,?,?,?,1,?,?,?,?,?,?,?,?,?,?,?,?)",
            (
                tenant["id"],
                tenant["company_name"],
                tenant["db_filename"],
...
def wipe_user_data(user: dict, password: str, phrase: str) -> dict:
    ...
    support_store_backup(user, backup.read_bytes(), backup.name)
    preserve = {
        ...
        try:
            ...
            if path.startswith("/api/admin/"):
                ...
                if path == "/api/admin/companies":
                    ...
                    return self.send_json({"companies": admin_list_companies(search)})
                # Company detail
                ...
                if path == "/api/admin/licenses":
                    return self.send_json({"licenses": admin_list_licenses()})
                if path == "/api/admin/license-orders":
                    status = query.get("status", [""])[0]
                    return self.send_json({"orders": list_license_orders(status=status)})
                # Backups list
                ...
                if "/api/admin/backups/" in path and "/download/" in path:
                    ...
                    if backup_id:
                        payload, _digest, filename = support_backup_file(user, backup_id)
                    else:
    ...
    def do_POST(self):
        try:
            ...
            if path.startswith("/api/admin/") and path != "/api/admin/wipe-user-data":
                ...
                # Update company
                if path.startswith("/api/admin/companies/") and not "/users" in path and not "/restore" in path:
                    tid = path.split("/api/admin/companies/")[-1]
            ...
            if path == "/api/support/backups/upload":
                try:
                    ...
                    return self.send_json(
                        support_store_backup(user, payload, self.headers.get("X-AYEC-Backup-Name", "desktop.db"))
                    )
                except PermissionError as error:
                    return self.send_json({"error": str(error)}, 403)
                except ValueError as error:
