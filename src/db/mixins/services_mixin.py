...
class ServicesMixin:
    ...
    def add_service(self, name, price, description="", barcode="", currency="TRY"):
        """Yeni hizmet ekle. Aynı isimde kayıt varsa fiyat/açıklama güncellenir."""
        try:
            created_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            # is_deleted fark etmeksizin aynı isimde kayıt var mı kontrol et
            self.cursor.execute(
                "SELECT id FROM services WHERE name=?", (name,)
            )
            existing = self.cursor.fetchone()
            if existing:
                # Varsa güncelle (fiyat, açıklama, barkod) ve is_deleted=0 yap
                existing_id = existing[0] if not hasattr(existing, "keys") else existing["id"]
                try:
                    self.cursor.execute("""
                        UPDATE services
                        SET price=?, description=?, barcode=?, currency=?, is_deleted=0
                        WHERE id=?
                    """, (price, description, barcode, currency or "TRY", existing_id))
                except Exception:
                    self.cursor.execute("""
                        UPDATE services
                        SET price=?, description=?, barcode=?
                        WHERE id=?
                    """, (price, description, barcode, existing_id))
                self.conn.commit()
                return existing_id
            # Yoksa INSERT
            try:
                self.cursor.execute("""
                    INSERT INTO services (name, price, description, barcode, currency, created_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (name, price, description, barcode, currency or "TRY", created_at))
            except Exception:
                self.cursor.execute("""
                    INSERT INTO services (name, price, description, barcode, created_at)
                    VALUES (?, ?, ?, ?, ?)
                """, (name, price, description, barcode, created_at))
            self.conn.commit()
        ...
        try:
            try:
                self.cursor.execute("""
                    ...
                    WHERE id=?
                """, (name, price, description, barcode, currency or "TRY", service_id))
            except Exception:
                self.cursor.execute("""
                    UPDATE services
                    SET name=?, price=?, description=?, barcode=?
                    WHERE id=?
                """, (name, price, description, barcode, service_id))
            self.conn.commit()
            return True
        except Exception as e:
